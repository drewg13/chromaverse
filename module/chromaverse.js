...
Hooks.once('init', async function() {
  ...
  // Register settings
  game.settings.register("chromaverse", "alt_d20_explosion", {
    name: "Alternate D20 Explosions",
    hint: "D20's explode as scaling trait dice rather than d20s",
    scope: "world",
    config: true,
    type: Boolean,
    choices: {
  ...
  Actors.registerSheet("chromaverse", olActorSheet, { types: ["character"], makeDefault: true });
  Actors.registerSheet("chromaverse", olNPCActorSheet, { types: ["npc"], makeDefault: true });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("chromaverse", olItemSheet, { makeDefault: true });
...
Hooks.once("ready", async function() {
  ...
  let statusEffects = [];
  const packBanes = await game.packs.get("chromaverse.banes").getDocuments();
  ...
  const packBoons = await game.packs.get("chromaverse.boons").getDocuments();
...
export const _getInitiativeFormula = function() {
  ...
  const agi = actor.system.traits.physical.agility.dice;
